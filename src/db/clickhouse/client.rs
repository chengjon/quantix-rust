//! `ClickHouseClient` — the shared HTTP/RowBinary client used by every
//! table-specific impl block in this module (`kline.rs`, `fundamentals.rs`,
//! `gbbq.rs`, `schema.rs`, `shadow_kline.rs`).
//!
//! Construction, connection check, batch-size knob, and the JSON-over-HTTP
//! fallback query path live here. Sibling files add `impl ClickHouseClient`
//! blocks for their respective tables.
