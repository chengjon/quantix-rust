...
impl ClickHouseClient {
    ...
    async fn create_stock_info_table(&self) -> Result<()> {
        ...
        self.client
            .query(render_cluster_ddl(sql).as_str())
            .execute()
            .await
            .map_err(|e| {
...
async fn execute_admin_sql(http_url: &str, user: &str, password: &str, sql: &str) -> Result<()> {
    ...
    let body = response
        .text()
        .await
        .map_err(|e| QuantixError::DatabaseConnection(format!("读取创建数据库响应失败: {}", e)))?;

    if !status.is_success() {
        return Err(QuantixError::DatabaseConnection(format!(
            "创建数据库失败: {}",
            body
        )));
    }
...
fn render_cluster_ddl(sql: &str) -> String {
    let cluster = std::env::var("CLICKHOUSE_CLUSTER")
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty());

    match cluster {
        Some(cluster) => sql.replace("'{cluster}'", &cluster),
        None => sql.replace(" ON CLUSTER '{cluster}'", ""),
    }
}

#[cfg(test)]
mod tests {
    use super::{execute_admin_sql, render_cluster_ddl};
    ...
    #[tokio::test]
    async fn execute_admin_sql_surfaces_http_errors_without_target_database_context() {
        let err = execute_admin_sql(
            "http://127.0.0.1:9",
            "default",
            "",
            "CREATE DATABASE IF NOT EXISTS quantix_smoke",
        )
        .await
        .unwrap_err();

        let message = err.to_string();
        assert!(message.contains("创建数据库失败"));
        assert!(!message.contains("UNKNOWN_DATABASE"));
    }

    #[test]
    fn render_cluster_ddl_omits_on_cluster_without_env() {
        let _lock = env_lock();
        unsafe { std::env::remove_var("CLICKHOUSE_CLUSTER") };

        let rendered =
            render_cluster_ddl("CREATE TABLE IF NOT EXISTS demo ON CLUSTER '{cluster}' (id UInt8)");

        assert_eq!(rendered, "CREATE TABLE IF NOT EXISTS demo (id UInt8)");
    }

    #[test]
    fn render_cluster_ddl_uses_cluster_env_when_present() {
        let _lock = env_lock();
        unsafe { std::env::set_var("CLICKHOUSE_CLUSTER", "quantix_cluster") };
        ...
        assert_eq!(
            rendered,
            "CREATE TABLE IF NOT EXISTS demo ON CLUSTER quantix_cluster (id UInt8)"
        );
