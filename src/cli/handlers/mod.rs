...
async fn execute_market_command_with_runtime<R>(
    cmd: MarketCommands,
    reader: R,
...
fn print_market_foundation_summary(summary: &MarketFoundationSummary) {
    println!("== 市场基础数据 ==");
...
fn print_market_strength_report(report: &MarketStrengthReport) {
    println!("== 强弱板块分析 ==");
    println!(
        "基础数据: A股={} 行业覆盖={} 未覆盖={}",
    ...
    println!();
    println!("强势板块个股 Top10 总市值:");
    print_strong_sector_stock_rows(&report.top_by_market_cap, "总市值(亿)", true);

    println!();
    println!("强势板块个股 Top10 最新净利润:");
    print_strong_sector_stock_rows(&report.top_by_profit, "净利润(亿)", false);
}
...
fn print_strong_sector_stock_rows(
    ...
    for (idx, row) in rows.iter().enumerate() {
        ...
        println!(
            ...
            row.name,
            row.latest_price,
            format_market_decimal(metric)
        );
    }
}

fn format_market_decimal(value: Option<Decimal>) -> String {
    match value {
