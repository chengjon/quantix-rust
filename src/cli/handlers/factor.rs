...
pub async fn run_factor_command(cmd: FactorCommands) -> Result<()> {
    match cmd {
        ...
        FactorCommands::Score {
            ...
            let result = crate::factor::score_factors_latest(&catalog, &dataset, &factors, top)?;
            match format {
                FactorOutputFormat::Table => {
                    println!("factors={}", result.factors.join(","));
                    println!("{}", result.frame);
                }
                FactorOutputFormat::Csv => {
                    let output = output.as_deref().ok_or_else(|| {
                        QuantixError::Config(
                            "factor score csv output requires --output <path>".to_string(),
                        )
                    })?;
                    std::fs::write(
                        output,
                        crate::factor::factor_score_result_to_csv_string(&result)?,
                    )?;
                }
                FactorOutputFormat::Json => {
                    let json = crate::factor::factor_score_result_to_json_string(&result)?;
                ...
                FactorOutputFormat::Parquet => {
                    let output = output.as_deref().ok_or_else(|| {
                        QuantixError::Config(
                            "factor score parquet output requires --output <path>".to_string(),
                        )
                    })?;
                    crate::factor::factor_score_result_to_parquet_file(&result, output)?;
                }
            }
            Ok(())
        }
