...
pub(crate) async fn run_execution_command_impl(cmd: ExecutionCommands) -> Result<()> {
    match cmd {
...
pub(crate) fn create_bridge_client() -> Result<BridgeHttpClient> {
    let runtime = CliRuntime::load();
...
async fn execute_execution_bridge_qmt_live(request_id: &str, skip_confirm: bool) -> Result<()> {
    ...
    let response = client
        .qmt_submit_order(&order_request)
        .await
