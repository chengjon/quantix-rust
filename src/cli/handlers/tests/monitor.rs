use super::*;
use super::support::{
    FakeMonitorAlertState, FakeMonitorAlertStore, FakeMonitorQuoteReader,
    FakeMonitorWatchlistReader, FakeStopRuleState, FakeStopRuleStore, monitor_alert,
    monitor_quote_row, monitor_sample_time, monitor_watchlist_item, stop_rule,
};
...
async fn test_execute_monitor_stop_trailing_triggers_after_drawdown() {
    ...
    let state = store.state.lock().unwrap();
    assert_eq!(state.rules[0].last_triggered_at, Some(monitor_sample_time()));
}
