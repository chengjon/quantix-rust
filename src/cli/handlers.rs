...
use crate::screener::{
    DailyKlineLoader, PresetInvocation, RuleMatchDetail, ScreenRow, ScreenRunOptions,
    ScreenSortBy, ScreenUniverse, ScreenerService, parse_preset_invocation,
};
use crate::stop::{
...
pub use self::{
    ...
    market::run_market_command,
    menu::{run_simple_menu, run_tui_menu},
    monitor::{run_monitor_command, run_stop_command},
    trade::run_trade_command,
    watchlist::run_watchlist_command,
};
use self::watchlist::{create_watchlist_storage, format_tags, load_watchlist_store_for_read};
#[cfg(test)]
use self::{
    market::{MarketCommandOutput, execute_market_command_with_reader},
    monitor::{
        MonitorCommandOutput, StopCommandOutput, execute_monitor_command_with_service,
        execute_monitor_command_with_stop_store, execute_stop_command_with_service,
        persist_triggered_monitor_alerts,
    },
    screener::{ScreenerCommandOutput, execute_screener_command_with_loader},
    trade::{TradeCommandOutput, execute_trade_command_with_service},
};
...
async fn export_data(code: String, format: String, output: String) -> Result<()> {
    ...
    let client = create_clickhouse_client().await?;
    let klines = client
        .get_kline_data(&code, "1d", None, None, Some(10000))
        .await?;
...
async fn start_task_scheduler(daemon: bool) -> Result<()> {
    ...
    if let Err(e) = scheduler.add_task(TaskTemplates::auction_collection()).await {
    ...
    if let Err(e) = scheduler.add_task(TaskTemplates::post_market_process()).await {
