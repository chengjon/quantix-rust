...
use crate::execution::models::{
    ApprovalStatus, ExecutionRequestRecord, ExecutionRequestStatus, SignalStatus,
};

use super::runtime_store_codec::build_execution_snapshot;
use super::runtime_store_rows::{row_to_execution_request, row_to_signal};

pub(crate) async fn approve_signal_and_create_request(
    pool: &SqlitePool,
    signal_id: &str,
    target_mode: &str,
    target_account: &str,
...
pub(crate) async fn reject_signal(
    pool: &SqlitePool,
    signal_id: &str,
    reason: Option<&str>,
) -> Result<()> {
...
pub(crate) async fn list_execution_requests(
    pool: &SqlitePool,
...
pub(crate) async fn get_execution_request_by_signal_id(
    pool: &SqlitePool,
    signal_id: &str,
...
pub(crate) async fn get_execution_request(
    pool: &SqlitePool,
    request_id: &str,
) -> Result<Option<ExecutionRequestRecord>> {
    let row = sqlx::query(
...
pub(crate) async fn try_complete_execution_request(
    pool: &SqlitePool,
    request_id: &str,
    payload_json: serde_json::Value,
    updated_at: DateTime<Utc>,
) -> Result<bool> {
    try_update_execution_request_status(
        pool,
        request_id,
...
pub(crate) async fn try_fail_execution_request(
    pool: &SqlitePool,
    request_id: &str,
    payload_json: serde_json::Value,
    updated_at: DateTime<Utc>,
...
pub(crate) async fn try_cancel_execution_request(
    pool: &SqlitePool,
    request_id: &str,
    payload_json: serde_json::Value,
    updated_at: DateTime<Utc>,
...
pub(crate) async fn try_start_execution_request(
    pool: &SqlitePool,
    request_id: &str,
    payload_json: serde_json::Value,
    updated_at: DateTime<Utc>,
...
pub(crate) async fn find_next_pending_execution_request(
    pool: &SqlitePool,
) -> Result<Option<ExecutionRequestRecord>> {
    let row = sqlx::query(
        r#"
...
pub(crate) async fn supersede_previous_signals_and_cancel_pending_requests(
    pool: &SqlitePool,
