...
pub(crate) async fn ensure_runtime_store_schema(pool: &SqlitePool) -> Result<()> {
    for statement in [
    ...
    ensure_orders_schema_extensions(pool).await?;

    Ok(())
}

async fn ensure_orders_schema_extensions(pool: &SqlitePool) -> Result<()> {
