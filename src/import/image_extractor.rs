...
impl ImageFormat {
    /// 返回该格式的标准 MIME 类型字符串（image/png / image/jpeg / image/gif / image/webp），用于上传 vision API。
    ...
    /// 由文件扩展名（不含点，大小写不敏感）推断 ImageFormat；png / jpg / jpeg / gif / webp 各自映射，未知扩展名返回 None。
...
impl ImageVisionProvider {
    /// 反向解析 vision provider 字符串（"deepseek" / "openai"，大小写不敏感）；未知值返回 QuantixError::Unsupported，错误信息列出受支持项。
...
impl ImageExtractor {
    /// 默认使用 Deepseek 作为 vision provider 构造提取器；等价于 with_provider(ImageVisionProvider::Deepseek)。
    pub fn new() -> Self {
        Self::with_provider(ImageVisionProvider::Deepseek)
    }

    /// 显式指定 vision provider（deepseek / openai）构造提取器；内部初始化 CodeResolver 与 reqwest::Client。
    ...
    /// 从 base64 数据提取
    /// 直接从 base64 编码的图片数据提取股票代码：调用 vision API，返回 ImportResult（items 字段含解析出的代码；total_input_lines=1，parsed_count/skipped_count=0，errors 为空——细粒度统计仅在 call_vision_api 内部产生）。API 调用失败透传。
