...
| `sources/` 数据源 | [部分实现] | `src/sources/`; `bridge_tdx`, `eastmoney`, `websocket` 等树节点 | AKShare/TDX 的 StockInfo/Kline 拉取、缠论适配仍不可用；EastMoney 资金流向字段映射未接线时 fail-closed，不返回占位全 0 数据 |
...
| `quantix data` | [部分实现] | CLI 命令树; `src/cli/commands/data.rs`; `src/cli/handlers/data_handler.rs` | 数据源管理、查询和 CSV/Parquet 导出可用；未知 export 格式会在读取数据或输出前 fail-closed 返回 Unsupported；数据完整性仍依赖已配置数据源 |
