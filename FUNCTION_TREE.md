...
| `monitoring/` 监控层 | [部分实现] | `src/monitoring/`; `src/cli/handlers/notify.rs` | 监控框架存在；多通知渠道未全部真正接线；`notify check/test --channel <外部渠道>` 缺渠道配置时 fail-closed |
...
| `quantix notify` | [部分实现] | CLI 命令树; `src/cli/handlers/notify.rs` | 仅部分渠道真正接线；`check/test --channel <外部渠道>` 缺必需环境变量时返回显式 `Unsupported`；`test --channel all` 保留聚合配置路径；Telegram/Discord/Slack/Dingtalk/Pushplus/Email 需按实现确认 |
...
| 多通知渠道 | [已设计/待实现] | 模块证据展开 `monitoring/notification`; CLI `quantix notify` 边界 | Telegram/Discord/Slack/Dingtalk/Pushplus/Email 不应被写成已接线能力；当前 `notify check/test` 对缺环境变量渠道 fail-closed，不代表渠道投递闭环完整 |
