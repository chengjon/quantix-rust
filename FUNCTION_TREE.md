...
| `monitoring/` 监控层 | [部分实现] | `src/monitoring/`; `src/cli/handlers/notify.rs` | 监控框架存在；多通知渠道未全部真正接线；`notify check` 缺渠道配置时 fail-closed |
...
| `quantix notify` | [部分实现] | CLI 命令树; `src/cli/handlers/notify.rs` | 仅部分渠道真正接线；`check --channel <外部渠道>` 缺必需环境变量时返回显式 `Unsupported`；Telegram/Discord/Slack/Dingtalk/Pushplus/Email 需按实现确认 |
...
| 多通知渠道 | [已设计/待实现] | 模块证据展开 `monitoring/notification`; CLI `quantix notify` 边界 | Telegram/Discord/Slack/Dingtalk/Pushplus/Email 不应被写成已接线能力；当前 `notify check` 对缺环境变量渠道 fail-closed，不代表渠道投递闭环完整 |
...
| 本地风控/监控/止损工作流 | [部分实现] | 注册表 `risk/`, `monitoring/`, `stop/`; CLI `risk/monitor/stop/notify` | 核心工作流可用；`notify check` 缺渠道配置时 fail-closed；通知渠道投递闭环和部分引用数据同步仍受限 |
