...
| `ai/` AI 决策层 | [部分实现] | `src/ai/`; `src/cli/handlers/ai.rs` | OpenAI-compatible、DeepSeek、Ollama 等路径已接线；`analyze`/`decide`/`ask`/`market` 会运行时提示模拟上下文、问答参数或固定 prompt 边界；Gemini/Anthropic 仅部分配置，conversation/skill_registry 不可用 |
