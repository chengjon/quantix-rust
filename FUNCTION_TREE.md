...
| `execution/` 执行层 | [部分实现] | `src/execution/`; `qmt_live_gate.rs`; `qmt_task_submit_service.rs`; `qmt_live_adapter.rs`; `paper.rs` | `paper/mock_live/qmt_live` 有路径；paper adapter 可按本地 `TradeRecord.id` 查询已成交记录，撤销已成交/不存在订单会 fail-closed；通用 `live` 不完整，`qmt_live` 受 gate 和 Windows Bridge 约束 |
