...
- 2026-06-01 behavior/docs sync: `quantix ai config --test` now reports configured provider status without claiming real API connectivity. It prints an explicit "configured, real connectivity test not performed" status instead of "available", and the CLI manual/help describe the command as a configuration check rather than a connectivity acceptance gate. The generated function/status registry above remains the source of feature truth.
