...
| `quantix algo` | [部分实现] | CLI 命令树; `src/cli/handlers/algo.rs`; `src/execution/algo/context.rs` | TWAP/VWAP 可用；`create`/`plan` 参数验证 fail-closed；任务为单进程内存态，POV/Iceberg 不可用 |
...
│   └── plan                # 预览切片计划（当前仅支持 TWAP/VWAP，参数无效时失败关闭）
│
├── ai                      # AI决策分析
