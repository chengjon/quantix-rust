...
| `ai/` AI 决策层 | [部分实现] | `src/ai/`; `src/cli/handlers/ai.rs` | OpenAI-compatible、DeepSeek、Ollama 等路径已接线；`analyze`/`decide`/`market` 会运行时提示模拟上下文或固定 prompt 边界；Gemini/Anthropic 仅部分配置，conversation/skill_registry 不可用 |
...
- 2026-06-01 behavior/docs sync: `quantix ai analyze`, `quantix ai decide`, and `quantix ai market` now print explicit runtime warnings when they use simulated price/indicator context, simulated technical-analysis context, or a fixed market prompt. README, CHANGELOG, CLI_COMMAND_MANUAL, and repo hygiene checks record that these entries validate LLM wiring rather than real-time research or live trading decisions. The generated function/status registry above remains the source of feature truth.
