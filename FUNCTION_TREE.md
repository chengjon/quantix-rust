...
| Execution mode semantics hardening | [部分实现] | `docs/reports/EXECUTION_MODE_SEMANTICS_HARDENING_PROPOSAL_2026-06-16.md`; `src/execution/mode_semantics.rs`; `tests/execution_mode_semantics_test.rs`; `.governance/programs/project-governance/cards/P0.2.yaml`; `.governance/programs/project-governance/cards/P0.2b.yaml`; `.governance/programs/project-governance/cards/P0.2c.yaml`; 注册表 `execution/` 边界 | 已完成语义硬化提案、`paper_immediate` 行为锁止、三类通道标准风险提示目录，以及 paper/qmt_live submit-time 风险提示日志接线；尚未改动 trait、状态响应、qmt_live 能力或 paper 模拟生命周期 |
