...
| `fundamental/` 基本面分析 | [部分实现] | `src/fundamental/`; `src/cli/handlers/fundamental.rs` | EastMoney/估值/财报部分可用；资金流向仍不可用，分红命令在真实数据源接线前 fail-closed 返回 Unsupported |
...
| 资金流向 / 分红基本面 | [已设计/待实现] | 模块证据展开 `fundamental/capital_flow`, `fundamental/dividend`; CLI `fundamental capital-flow/dividend` 边界 | 当前不可用；`dividend` 命令壳已 fail-closed 返回 Unsupported，不得写成已可查询数据 |
...
└── 📟 CLI 命令 (cli/)
    └── 命令处理器 (handlers)
        ...
        ├── ai - AI 决策
        ├── news - 新闻搜索
        ├── fundamental - 基本面分析（分红 fail-closed，资金流向 CLI 未暴露）
        ├── sentiment - 舆情分析
        ├── notify - 多渠道通知
        ├── import - 智能导入
        └── status - 系统状态
