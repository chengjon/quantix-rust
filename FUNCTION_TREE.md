...
| Paper query/cancel | [部分实现] | `src/execution/paper.rs`; `tests/execution_kernel_test.rs`; 模块证据展开 `execution/paper`; 注册表 `execution/` 边界 | paper adapter 可查询本地已成交 `TradeRecord`，并对已成交/不存在订单返回显式 fail-closed 错误；当前仍不是挂单撮合或可撤未成交订单生命周期 |
| Execution mode semantics hardening | [已设计/待实现] | `docs/reports/EXECUTION_MODE_SEMANTICS_HARDENING_PROPOSAL_2026-06-16.md`; `.governance/programs/project-governance/cards/P0.2.yaml`; 注册表 `execution/` 边界 | 当前仅完成语义硬化提案：固定 `paper_immediate`、`paper_sim_lifecycle`、`qmt_live` 的成交来源/状态来源/存储隔离/错误分层边界；尚未改动 trait、状态响应、qmt_live 能力、paper 模拟生命周期或运行时代码 |
