...
| `quantix data` | [部分实现] | CLI 命令树; `src/cli/commands/data.rs`; `src/cli/handlers/data_handler.rs` | 数据源管理、查询和 CSV/Parquet 导出可用；数据完整性仍依赖已配置数据源 |
...
│   ├── query               # 查询历史数据
│   └── export              # 导出数据 [CSV/Parquet 可用；输出依赖数据源配置和本地写入权限]
│
