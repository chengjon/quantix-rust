...
| `quantix notify` | [部分实现] | CLI 命令树; `src/cli/handlers/notify.rs` | 仅部分渠道真正接线；`check/test/send --channel <外部渠道>` 对未知渠道或缺必需环境变量返回显式 `Unsupported`；`test --channel all` 保留聚合配置路径；Telegram/Discord/Slack/Dingtalk/Pushplus/Email 需按实现确认 |
