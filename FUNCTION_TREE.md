...
| `ai/` AI 决策层 | [部分实现] | `src/ai/`; `src/cli/handlers/ai.rs` | OpenAI-compatible、DeepSeek、Ollama 等路径已接线；`analyze`/`decide`/`ask`/`market` 会运行时提示模拟上下文、问答参数或固定 prompt 边界；Gemini/Anthropic 仅有配置/模型枚举，runtime 命令会 fail-closed，不会静默回退到 Ollama；conversation/skill_registry 不可用 |
...
| `quantix ai` | [部分实现] | CLI 命令树; `src/cli/handlers/ai.rs` | 决策分析/问答入口可用；runtime provider 仅 DeepSeek/OpenAI/Ollama 已接线，Gemini/Anthropic 配置存在但未接线时 fail-closed；多轮 conversation 和技能包管理不可用 |
...
│
├── ai                      # AI决策分析
│   ├── analyze             # AI 分析股票（使用模拟分析上下文，运行时显式提示）
│   ├── decide              # AI 交易决策（基于模拟分析上下文，运行时显式提示）
│   ├── ask                 # 对话式分析（provider 自动选择，--code/--model 边界运行时显式提示）
│   ├── market              # 市场整体分析（固定 prompt，运行时显式提示）
│   └── config              # AI配置管理
│       ├── --show          # 显示当前配置
