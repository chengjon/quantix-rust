...
| Execution mode semantics hardening | [部分实现] | `docs/reports/EXECUTION_MODE_SEMANTICS_HARDENING_PROPOSAL_2026-06-16.md`; `.governance/programs/project-governance/cards/P0.2.yaml`; `.governance/programs/project-governance/cards/P0.2b.yaml`; 注册表 `execution/` 边界 | 已完成语义硬化提案和 `paper_immediate` 行为锁止：固定 `paper_immediate`、`paper_sim_lifecycle`、`qmt_live` 的成交来源/状态来源/存储隔离/错误分层边界，并防止 paper adapter 引入 pending 生命周期；尚未改动 trait、状态响应、qmt_live 能力或 paper 模拟生命周期 |
