...
| `import/` 智能导入 | [部分实现] | `src/import/`; `src/cli/handlers/import.rs`; `src/cli/tests/import.rs` | 现有智能导入路径可用；`from-excel` 已接入 watchlist 级 Excel worksheet 解析，复杂 Excel schema 与持久化导入闭环仍不可用 |
...
| `quantix import` | [部分实现] | CLI 命令树; `src/cli/handlers/import.rs`; `src/import/excel_parser.rs` | 智能导入入口存在；Excel 文件导入已支持 watchlist worksheet 解析；`market-manifest` 仍是 artifact dry-run resolution，不执行数据库写入 |
