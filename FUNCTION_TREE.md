...
| `import/` 智能导入 | [部分实现] | `src/import/`; `src/cli/handlers/import.rs`; `src/cli/tests/import.rs` | 现有智能导入路径可用；`from-image` 仅支持 `png, jpg, jpeg, gif, webp`，未知扩展名会在 Vision provider 配置校验前 fail-closed；`from-image --model deepseek|openai` 按所选 Vision provider 配置请求并在缺 API key 时 fail-closed；`from-excel` 已接入 watchlist 级 Excel worksheet 解析，复杂 Excel schema 与持久化导入闭环仍不可用 |
...
| `quantix import` | [部分实现] | CLI 命令树; `src/cli/handlers/import.rs`; `src/import/excel_parser.rs`; `src/import/image_extractor.rs` | 智能导入入口存在；`from-image` 未知扩展名会在 Vision provider 配置校验前返回显式 `Unsupported` 和支持格式清单；`from-image --model deepseek|openai` 不再是显示字段，会按所选 Vision provider 配置请求并在缺 API key 时返回显式 `Unsupported`；Excel 文件导入已支持 watchlist worksheet 解析；`market-manifest` 仍是 artifact dry-run resolution，不执行数据库写入 |
...
│   │   └── LLM Vision识别（deepseek/openai provider 选择；缺 API key fail-closed）
│   ├── CSV解析 (csv_parser)
