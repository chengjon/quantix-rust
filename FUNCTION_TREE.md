...
| `import/` 智能导入 | [部分实现] | `src/import/`; `src/cli/handlers/import.rs`; `src/cli/tests/import.rs` | 现有智能导入路径可用；`from-excel` 命令壳已 fail-closed 返回 Unsupported，真实 Excel parser 尚不可用 |
...
| `quantix import` | [部分实现] | CLI 命令树; `src/cli/handlers/import.rs` | 智能导入入口存在；Excel 文件导入命令壳已暴露但真实解析器尚不可用 |
...
| Excel 文件导入 | [已设计/待实现] | 模块证据展开 `import/excel_parser`; CLI `import from-excel` 边界 | 当前不可用；`from-excel` 命令壳已 fail-closed 返回 Unsupported，智能导入不包含 Excel 解析闭环 |
...
└── import                  # 智能导入 [可选 - Phase 5]
    ├── from-image          # 图片识别导入
    ├── from-csv            # CSV文件导入
├── from-excel          # Excel文件导入（命令壳返回 Unsupported）
...
- 2026-06-01 behavior/docs sync: `quantix import from-excel` is now exposed as a CLI command shell and fail-closes with explicit `Unsupported` until a real Excel parser is wired. README, CHANGELOG, CLI_COMMAND_MANUAL, and repo hygiene checks were updated to replace the stale "no CLI entry" boundary while preserving the `Excel 文件导入` node as `[已设计/待实现]`. The generated function/status registry above remains the source of feature truth.
