...
| Carried TUI finding | `menu tui app CLI advertised TUI development in progress` | Found `run_tui_menu` and CLI `Menu { tui }` dispatch. | Kept `AUDIT-S3-009` open. |
...
| `run_tui_menu` | `src/cli/handlers/app_shell.rs` | Incoming call from CLI `run`; outgoing graph is empty; implementation prints an in-progress message and returns `Ok(())`. |
