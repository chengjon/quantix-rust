...
>
> **R1 修订（2026-07-03）**：修正 `fetch<T>()` 调用签名（双参数 `category + params`）、`RawMinuteRecord` 直接用 `Option<Decimal>`、handler date 变量 shadow、`OpenStockResponse` 类型名、`base_url` Option 解包。审核意见：`2026-07-02-openstock-p0-13b-2-design-review.md`。
...
CLI: data openstock fetch-minute-share --symbol sh600000 --date 2026-06-30
   ↓
Handler: fetch_openstock_minute_share(settings, symbol, date_str)
   ↓  OpenStockClient::from_settings(settings)?
   ↓  NaiveDate::parse_from_str(date_str, "%Y-%m-%d")
OpenStockClient::fetch_minute_share(code, date)
   ↓  self.fetch::<RawMinuteRecord>("MINUTE_DATA", params).await
   ↓    params = { "code": code, "date": "YYYY-MM-DD" }
   ↓  fetch<T>() 内部拼装 body = { data_category: "MINUTE_DATA", params: {...} }
   ↓  POST /data/fetch（envelope + retry + circuit breaker）
OpenStockResponse<RawMinuteRecord>（response.records 是 8 字段数组）
   ↓  parse_minute_share(code, &raw, date) -> Option<MinuteShare>
...
///
/// **数值字段直接用 `Decimal`**（rust_decimal + serde feature 自动
/// 反序列化 JSON number），消除 `from_f64_retain` 中间跳与 R3 精度
/// 风险。如果 live 测试发现 OpenStock 返回字符串格式数值（如 "10.50"），
/// 则需要切换到 `serde_json::Value` + 已有 `parse_decimal` helper
/// （见 §6 D2 风险缓解）。
...
/// `{data_category: ..., params: ...}` envelope。**不要**手工拼装
/// `{category: "MINUTE_DATA", ...}` 单 body——签名不匹配，编译失败。
///
...
///
/// **数值字段**：`RawMinuteRecord` 已直接用 `Option<Decimal>`，parser
/// 仅 `?` 解包，无 `from_f64_retain` 中间跳。
...
**变量命名说明**：参数 `date_str`（用户输入的原始字符串）与解析后的 `date: NaiveDate` 分开命名，避免 shadow 导致的语义混淆。
...
**决策**：定义显式 struct `RawMinuteRecord` 带所有 8 字段（保留 + 丢弃），数值字段直接用 `Option<Decimal>`，丢弃字段用 `#[serde(default)]` 容忍缺失。

**原因**：
- 显式 struct 比 `serde_json::Value` 性能更好（无运行时字段查找）
- 编译时字段名检查
- **数值字段直接用 `Decimal`**：`rust_decimal` 的 serde feature 自动将 JSON number 反序列化为 `Decimal`，消除 `from_f64_retain` 中间跳和 R3 精度损失风险
- 与 P0.13b-1 的 `RawMinuteBar` 模式（如果存在）保持一致

**风险缓解（string 数值漂移）**：如果 live 测试发现 OpenStock MINUTE_DATA 返回字符串格式数值（如 `"10.50"` 而非 `10.50`，类似 P0.9 `IndexKlineRecord` 教训），则需要切换到 `serde_json::Value` + 已有 `parse_decimal` helper（复刻 `src/sources/openstock_index.rs:152-176`）。wiremock 测试假设 JSON number，live 测试验证此假设。

**替代方案（拒绝）**：
- 用 `serde_json::Value` + 手动 `get()`：性能差，类型不安全
- 用 `Option<f64>` + `from_f64_retain`：引入精度风险且代码冗长
...
### R3：数值字段 serde 兼容性
**风险**：`RawMinuteRecord` 假设 OpenStock MINUTE_DATA 返回 JSON number（如 `10.50`）。若实际为字符串（`"10.50"`），serde + Decimal 反序列化失败。

**缓解（D2 已决策）**：
- wiremock 测试假设 JSON number，覆盖正常路径
- live 测试（`#[ignore]`）验证生产 schema
- 若发现 string 格式，切换到 `serde_json::Value` + `parse_decimal` helper（同 PR 修复）
