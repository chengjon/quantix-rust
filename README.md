...
- `quantix notify check --channel <外部渠道>` 和 `quantix notify test --channel <外部渠道>` 在缺少必需环境变量时返回显式 `Unsupported`，错误包含 `notify channel 尚未配置`；`notify test --channel all` 仍按环境聚合渠道发送，`notify list` 仍只是渠道名称状态视图
  - 系统通知延后到后续 Phase
