...
#[test]
fn market_sector_rejects_unsupported_sort_by_at_binary_entry() {
    let (stdout, stderr, success) = run_quantix(&["market", "sector", "--sort-by", "volume"]);
...
#[test]
fn market_north_rejects_invalid_date_at_binary_entry() {
    let (stdout, stderr, success) = run_quantix(&["market", "north", "--date", "20260309"]);
    ...
    assert!(
        !stdout.contains("沪股通"),
        "expected no normal north-flow output on invalid date, stdout={stdout}"
    );
}
