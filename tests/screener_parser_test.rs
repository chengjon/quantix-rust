...
fn parses_close_above_ma_preset_with_period() {
    ...
    assert_eq!(invocation.params.get("period").map(String::as_str), Some("20"));
}
...
fn parses_rsi_gte_preset_with_period_and_value() {
    ...
    assert_eq!(invocation.params.get("period").map(String::as_str), Some("14"));
    assert_eq!(invocation.params.get("value").map(String::as_str), Some("55"));
}
...
fn parses_volume_ratio_gte_with_decimal_value() {
    ...
    assert_eq!(invocation.params.get("window").map(String::as_str), Some("5"));
    assert_eq!(invocation.params.get("value").map(String::as_str), Some("1.5"));
}
